    ...
    <div class="container">
        <div class="row my-5 d-flex flex-column justify-content-between">
            ...
            <div class="col my-5">
                ...
                <p class="mb-0 text-center">{{ isset($_GET['month']) && isset($_GET['year']) ? 'Data pembelian pada
                    '.strftime('%B', mktime(0, 0, 0, $_GET['month'], 1)).' '.$_GET["year"] : (isset($_GET['month']) ?
                    'Data pembelian pada '.strftime('%B', mktime(0, 0, 0, $_GET['month'], 1)) : (isset($_GET['year']) ?
                    'Data pembelian pada '.$_GET["year"] : (isset($_GET["data"]) && $_GET["data"] == 'all' ? 'Data
                    pembelian keseluruhan' : (isset($_GET["data"]) && $_GET["data"] == 'today' ? 'Data pembelian hari
                    ini' : (isset($_GET["data"]) && $_GET["data"] == 'thisMonth' ? 'Data pembelian bulan ini' : 'bau'
                    ))))) }}
                </p>
            </div>
        </div>
        <table class="table my-5">
            <thead>
                <tr>
                    ...
                    <th scope="col">per Item/Kg</th>
                    <th scope="col">Biaya Angkut</th>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $item->nama_brg }}</td>
                    <td>{{ $item->jumlah }} Item/Kg</td>
                    <td>Rp. {{ number_format($item->satuan, 0, ',', '.') }}</td>
                    <td>Rp. {{ number_format($item->biaya_agkt, 0, ',', '.') }}</td>
