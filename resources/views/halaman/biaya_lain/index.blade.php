...
@section('container')
<div class="col-auto">
    <h1 class="app-page-title mb-0">Biaya Lain-Lain</h1>
</div>
<div class="col-10">
    <div class="page-utilities">
        <div class="row g-2 justify-content-start justify-content-md-end align-items-center">
            <div class="col-auto">
                <form action="/biaya-lain" method="GET" class="table-search-form row gx-1 align-items-center">
                    <div class="col-auto">
            ...
            <div class="col-auto">
                <a class="btn app-btn-primary"
                    href="/laporan-biaya-lain?{{ isset($_GET['month']) && isset($_GET['year']) ? 'month='.$_GET['month'].'&year='.$_GET['year'] : (isset($_GET['month']) ? 'month='.$_GET['month'] : (isset($_GET['year']) ? 'year= '.$_GET['year'] : 'data=all')) }}"
                    ...
                    <table class="table app-table-hover mb-0 text-left">
                        <thead>
                            <tr>
                                ...
                                <th class="cell">Tgl. Transaksi</th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="cell">{{$item->kode_biayalain}}</td>
                                @php
                                $tb = strtotime($item->tgl_transaksi);
                                @endphp
                                <td class="cell">
                                    {{date('d M Y', $tb)}}
                                </td>
                                ...
                                <td class="cell">
                                    <form action="{{ route('biaya-lain.destroy', $item->id) }}" method="POST">
                                        <input type="hidden" value="{{ $item->id }}" id="biayalain_id">
                                        <a id="show-biayalain" class="btn btn-outline-info btn-xs"
                                            data-bs-toggle="modal" data-bs-target="#biayaLainDetail" role="button"><i
                                                class="fa fa-eye"></i>
                                            Detail</a>
                                        <a class="btn btn-outline-warning btn-xs"
                                            href="{{ route('biaya-lain.edit',$item->id) }}">
...
<div class="modal fade" style="margin-top: 9%; margin-left: 9%;" data-bs-backdrop="static" data-bs-keyboard="false"
    id="biayaLainDetail" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h1 class="modal-title fs-5" id="exampleModalLabel">Detail Biaya Lain-Lain</h1>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body p-0" id="biayalain-body">
...
<script src="/js/show-biayalain.js"></script>
<script>
      ...
      elem.addEventListener('click', function(event) {
        ...
        if (queryString) {
            ...
            if (id == 'orders-paid-tab') {
                print.href = '/laporan-biaya-lain?data=today';
            } else if(id == 'orders-pending-tab') {
                print.href = '/laporan-biaya-lain?data=thisMonth';
            } else {
                print.href = '/laporan-biaya-lain?data=all';
            }
            return false;
        }
